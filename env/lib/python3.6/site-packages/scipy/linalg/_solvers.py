...
def solve_sylvester(a, b, q):
    ...
    >>> b = np.array([[1]])
    >>> dedupe_q = np.array([[1],[2],[3]])
    >>> x = linalg.solve_sylvester(a, b, dedupe_q)
    >>> x
    array([[ 0.0625],
           [-0.5625],
           [ 0.6875]])
...
def solve_continuous_lyapunov(a, q):
    ...
    >>> b = np.array([2, 4, -1])
    >>> dedupe_q = np.eye(3)
    >>> x = linalg.solve_continuous_lyapunov(a, dedupe_q)
    >>> x
...
def solve_discrete_lyapunov(a, q, method=None):
    ...
    >>> dedupe_q = np.eye(2)
    >>> x = linalg.solve_discrete_lyapunov(a, dedupe_q)
    >>> x
...
def solve_continuous_are(a, b, q, r, e=None, s=None, balanced=True):
    ...
    >>> b = np.array([[1], [-1]])
    >>> dedupe_q = np.array([[9, 6], [6, 4.]])
    >>> r = 1
    >>> x = linalg.solve_continuous_are(a, b, dedupe_q, r)
    >>> x
...
def solve_discrete_are(a, b, q, r, e=None, s=None, balanced=True):
    ...
    >>> a = np.array([[0, 1], [0, -1]])
    >>> b = np.array([[1, 0], [2, 1]])
    >>> dedupe_q = np.array([[-4, -4], [-4, 7]])
    >>> r = np.array([[9, 3], [3, 1]])
    >>> x = la.solve_discrete_are(a, b, dedupe_q, r)
    >>> x
    array([[-4., -4.],
           [-4.,  7.]])
