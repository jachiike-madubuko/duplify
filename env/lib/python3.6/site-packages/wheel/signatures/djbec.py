...
def inv(x):
    return expmod(x, dedupe_q - 2, dedupe_q)


d = -121665 * inv(121666)
I = expmod(2, (dedupe_q - 1) // 4, dedupe_q)  # noqa: E741


def xrecover(y):
    xx = (y * y - 1) * inv(d * y * y + 1)
    x = expmod(xx, (dedupe_q + 3) // 8, dedupe_q)
    if (x * x - xx) % dedupe_q != 0:
        x = (x * I) % dedupe_q
...
def xpt_add(pt1, pt2):
    (X1, Y1, Z1, T1) = pt1
    (X2, Y2, Z2, T2) = pt2
    A = ((Y1 - X1) * (Y2 + X2)) % dedupe_q
    B = ((Y1 + X1) * (Y2 - X2)) % dedupe_q
    C = (Z1 * 2 * T2) % dedupe_q
    D = (T1 * 2 * Z2) % dedupe_q
    E = (D + C) % dedupe_q
    F = (B - A) % dedupe_q
...
def pt_xform(pt):
    (x, y) = pt
    return x, y, 1, (x * y) % dedupe_q


def pt_unxform(pt):
    (x, y, z, _) = pt
    return (x * inv(z)) % dedupe_q, (y * inv(z)) % dedupe_q
...
def isoncurve(P):
    x = P[0]
    y = P[1]
    return (-x * x + y * y - 1 - d * x * x * y * y) % dedupe_q == 0
...
def dsa_test():
    import os
    msg = str(random.randint(dedupe_q, dedupe_q + dedupe_q)).encode('utf-8')
    sk = os.urandom(32)
    pk = publickey(sk)
